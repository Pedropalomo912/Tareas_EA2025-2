---
title: "mapas_y_descripciones_graficas_zacatecas"
...
# Mapas del estado de Zacatecas

## Datos Espaciales en R: Análisis y Visualización de Shapefiles
...
Este conjunto de datos para el estado de Zacatecas incluye: - Límites estatales (archivos `14ent.*`) - Límites municipales (archivos `14mun.*`) - Otras divisiones territoriales
...
Primero, leemos el archivo shapefile para Zacatecas:

```{r entidades}
# Nota: Asegúrate de que todos los archivos (shp, shx, dbf, prj, cpg) estén en el mismo directorio: data

Zacatecas_estado <- st_read("data/32ent.shp", options = "ENCODING=LATIN1", quiet = TRUE)
```

### Visualización del estado de Zacatecas

```{r mapa_Zacatecas_estado}
# Crear mapa básico de Zacatecas
mapa_Zacatecas <- ggplot(Zacatecas_estado) +
...
# Explorar los datos
cat("Número de municipios en Zacatecas:", nrow(municipios_Zacatecas), "\n")
cat("Primeros 10 municipios:\n")
print(municipios_Zacatecas$NOMGEO[1:10])
```
...
municipios_importantes <-municipios_Zacatecas$NOMGEO [c(1, 5, 10, 15, 20)] 

# Crear una variable para destacar estos municipios
municipios_Zacatecas <- municipios_Zacatecas %>%
  mutate(
...
mapa_destacados <- ggplot(municipios_Zacatecas) +
...
municipios_grandes <- municipios_Zacatecas |> 
  st_drop_geometry() |> 
...
mapa_areas <- ggplot(municipios_Zacatecas) +
...
# Descripciones graficas del estado de Zacatecas
...
#municipios_zacatecas <- st_read("1_estadistica_descriptiva/data/14mun.shp", options = "ENCODING=LATIN1", quiet = TRUE)
municipios_zacatecas <- st_read("data/32mun.shp", options = "ENCODING=LATIN1", quiet = TRUE)
...
ggplot(datos) +
  ...
  scale_fill_viridis_c(
    name = "Derechohabientes IMMS (%) de los municipios de Zacatecas",
    option = "C"
  ) +
  labs(
    title = "Derechohabientes a IMMS en el Estado de Zacatecas",
    caption = "Fuente: INEGI - Marco Geoestadístico"
  ) +
  theme_void() +
  theme(
